...
namespace VTerrain
{
    std::string Shaders::m_defaultVertexShader = std::string(
        ...
        "   fog = min(pow(distance(vec3(0,0,0), outPos.xyz), 2)/pow(fog_distance,2), 0.6f);\n"
        "   UV = texCoord;"
        "}\n"
    );
    ...
    std::string Shaders::m_defaultFragmentShader = std::string(
        ...
        "void main()\n"
        "{\n"
		"   float val = max(lightIntensity * (pos.y / max_height), fog);\n"
        "   if(UV.x < 0.005f || UV.x > 0.995f || UV.y < 0.005f || UV.y > 0.995f) { val = 1.f;}\n"
