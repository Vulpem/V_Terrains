...
namespace VTerrain
{
    ...
    PerlinNoise::NoiseMap PerlinNoise::GenNoiseMap(int offsetX, int offsetY)
    {
        ...
        for (uint y = 0; y < Config::chunkHeight + 1; ++y)
        {
            ...
            {
                ret.Data()[x + y * (Config::chunkWidth + 1)] = GetValue(x - offsetX * Config::chunkWidth, y - offsetY * Config::chunkHeight);
            }
        }
        return ret;
    }
    ...
    {
        float dx = static_cast<float>(Config::chunkWidth + 1) / Config::Noise::frequency;
        float dy = static_cast<float>(Config::chunkHeight + 1) / Config::Noise::frequency;
