...
void MaterialCreatorWindow::Draw()
{
	...
	ImGui::InputText("###save_path", (char*)save_path.data(), save_path.capacity());
...
void MaterialCreatorWindow::SetUniforms()
{
	...
	ImGui::InputText("###variable_name", (char*)uniform_name.data(), uniform_name.capacity());
...
void MaterialCreatorWindow::PrintUniforms()
{
	...
	for (vector<Uniform*>::iterator it = material.uniforms.begin(); it != material.uniforms.end(); ++it)
	{
		...
		case U_SAMPLER2D:
			...
			memcpy((char*)sampler_name.data(), (*it)->value + sizeof(int), sampler_size);
